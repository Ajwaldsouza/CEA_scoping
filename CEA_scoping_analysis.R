...
# 1. GENERAL BIBLIOMETRIC DATA GENERATION with bibliometrix
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
years[,"cum_count"] <- cumsum(years$count) # generate cumulative publication data
...
#''''''''''''''''''''''''''
# using the pre-generated web of science categories data downloaded from the database
...
#_______________________________________________________________________________
# 2. KEYWORD ANALYSES with bibliometrix
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Analysing/plotting the keyword trend and plotting the most occurring keywords 
# in each research domain (identified by clustering analysis of with VOSviewer)
...
#Generate topic trend of author keywords (DE) using fieldByYear
trend <- fieldByYear(data_bib, field = "DE", min.freq = 5, n.items =5, graph = F,
                     synonyms = synonym)


#extracting data to be used for plotting from the output object
data_trend_graph <- trend$df_graph%>%as_tibble()
...
# AUTHOR KEYWORDS (supplementary info)
#''''''''''''''''''''''''''''''''''''
# Creating a function to subset master dataset to the specified authors and 
# generate bibliometrics, including keywords
author_summary <-
  function(x) {
    data_bib %>%
      filter(grepl(x, AU)) %>%
...
# Creating individual strings with study ids for each focus level
...
# ANNUAL DOMAIN PROGRESS-HEATMAP
#'''''''''''''''''''''''''''''''
focus_year <- read_excel("Data/study_records/focus_year.xlsx", sheet = "Sheet1")

#Create counts normalised by annual publication counts
